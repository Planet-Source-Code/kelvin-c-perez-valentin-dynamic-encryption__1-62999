VERSION 5.00
Begin VB.UserControl OsenXPButton 
   AutoRedraw      =   -1  'True
   ClientHeight    =   450
   ClientLeft      =   0
   ClientTop       =   0
   ...
   PropertyPages   =   "cmdOsenXP.ctx":0000
   ScaleHeight     =   30
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   127
   ToolboxBitmap   =   "cmdOsenXP.ctx":002F
   ...
   End
End
Attribute VB_Name = "OsenXPButton"
...
Private cFace As Long, cLight As Long, cHighLight As Long, cShadow As Long, cDarkShadow As Long, strText As Long, strTextO As Long, cFaceO As Long, cMask As Long, XPFace As Long
...
Private Sub UserControl_InitProperties()
    ...
    MyButtonType = 3
    Call SetColors
    BackC = vbWhite 'cFace&HE0E0E0   ':
    BackO = BackC '&HC0E0FF   '
    ForeC = strText
    ForeO = vbBlue 'ForeC
    MaskC = &HC0C0C0
    Call CalstrTextRects
    useHand = True
...
If showFocusR And hasFocus Then
    SetTextColor UserControl.hdc, strText
...
If MyColorType = Custom Then
    ...
    strText = ConvertFromSystemColor(ForeC)
    strTextO = ConvertFromSystemColor(ForeO)
...
ElseIf MyColorType = [Use Container] Then
    ...
    cFaceO = cFace
    strText = GetTextColor(GetDC(GetParent(hWnd)))
    strTextO = strText
    cShadow = ShiftColor(cFace, -&H40)
    cLight = ShiftColor(cFace, &H1F)
...
'if MyColorType is 1 or has not been set then use windows colors
    ...
    strText = GetSysColor(COLOR_BTNTEXT)
    strTextO = strText
    OXPb = GetSysColor(COLOR_HIGHLIGHT)
    OXPf = ShiftColorOXP(OXPb)
End If
...
Select Case State 'in this select case, we only change the text color and draw only text that needs rc2, at the end, text that uses rc will be drawn
    Case 0 'normal caption
        txtFX rc
        SetTextColor .hdc, strText
    Case 1 'hover caption
        txtFX rc
        SetTextColor .hdc, strTextO
    Case 2 'down caption
        txtFX rc2
        If MyButtonType = Mac Then SetTextColor .hdc, cLight Else SetTextColor .hdc, strTextO
...
Public Property Let ImageIndex(ByVal New_ImageIndex As Integer)
    ...
    If New_ImageIndex > frmImages.IMG.ListImages.Count Then
        ...
        m_ImageIndex = New_ImageIndex
        PropertyChanged "ImageIndex"
        Set picNormal = frmImages.IMG.ListImages(113).Picture
        Call CalcPicSize
        Call CalstrTextRects
        Call Redraw(lastStat, True)
        PropertyChanged "PICN"
    Else
        If New_ImageIndex > 0 Then
            If New_ImageIndex <= frmImages.IMG.ListImages.Count Then
                Set picNormal = frmImages.IMG.ListImages(New_ImageIndex).Picture
                Call CalcPicSize
                Call CalstrTextRects
                Call Redraw(lastStat, True)
                PropertyChanged "PICN"
